...
app.get("/api/admin/users",async (req,resp)=>{
  ...
  let data = await User.find({},'firstName surName email tel dob role', function(err, docs){
    if(err){
      resp.send(err);
    }
    resp.json(docs );
...
User.findOne({_id:uId, role:'super-admin'},function(err, docs){

if(docs){
User.findOneAndUpdate({_id:userId},{$set:{role:role}},{
